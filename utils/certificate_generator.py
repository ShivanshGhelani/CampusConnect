...
class CertificateGenerator:
    ...
    async def generate_and_download_certificate(
        ...
        """
        try:
            print(f"DEBUG: Starting robust certificate generation for event {event_id}, student {enrollment_no}")
            ...
            if not self._is_eligible_event(event):
                ...
                return False, f"Certificate generation is under development for {registration_type.title()} {registration_mode.title()} events. Currently only supports Individual events (Free or Paid).", None
    ...
    async def _generate_certificate_pdf(
        ...
        try:
            ...
            pdf_bytes = await self._convert_html_to_pdf_multiple_methods(
                temp_html_path, processed_content, full_name, event.get('event_name', 'Event')
            )
    ...
    def _create_pdf_with_reportlab(self, student_name: str, event_name: str) -> bytes:
        ...
        story.append(Paragraph(f"Date: {datetime.now().strftime('%B %d, %Y')}", content_style))
    ...
    def _create_simple_pdf(self, student_name: str, event_name: str) -> bytes:
        ...
        p.drawCentredText(width/2, height-450, f"Date: {datetime.now().strftime('%B %d, %Y')}")
    ...
    async def _send_certificate_email_with_attachment(
        ...
        try:
            ...
            safe_student_name = "".join(c for c in full_name if c.isalnum() or c in (' ', '-', '_')).strip()
            safe_event_name = "".join(c for c in event.get('event_name', 'Event') if c.isalnum() or c in (' ', '-', '_')).strip()
            ...
            asyncio.create_task(self._delayed_cleanup([temp_pdf_path], delay_seconds=60))
    ...
    def create_certificate_download_response(self, pdf_bytes: bytes, file_name: str) -> StreamingResponse:
...
async def generate_certificate_for_student(event_id: str, enrollment_no: str) -> Tuple[bool, str, Optional[bytes]]:
    """
...
def create_certificate_download_response(pdf_bytes: bytes, filename: str) -> StreamingResponse:
    """
